...
router.get('/', async (req, res) => {
  ...
  if (!url) return res.status(400).json({ status: false, creator: 'YuiAPI', error: 'Debes proporcionar una URL de Facebook.' });
  try {
  ...
  } catch { res.status(502).json({ status: false, creator: 'YuiAPI', error: 'No se pudo procesar Facebook.' }); }
});
